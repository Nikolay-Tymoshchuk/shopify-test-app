export async function getOffers(accessToken, targetProductId) {
  ...
  console.log(
    "metafieldsResponse==>",
    metafieldsResponse.data.product.metafields.edges,
  );

  const metafields = metafieldsResponse.data.product.metafields.edges.map(
    (edge) => edge.node,
  );
  ...
  const offers = await Promise.all(
    ...
    `;
      ...
      console.log(
        "variantResponse==>",
        variantResponse.data.productVariant.product.featuredMedia,
      );
