...
export default function FunnelsForm() {
  ...
  const updateFormState = (key: string, value: string | number, delay: number) => {
  ...
  useEffect(() => {
    return updateFormState('title', titleValue, 200);
  }, [titleValue]);

  useEffect(() => {
    return updateFormState('discount', Number(discountValue), 500);
  }, [discountValue]);
